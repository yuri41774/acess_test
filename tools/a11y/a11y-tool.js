/* A11yTool - lightweight client library (UMD + ESM companion)
   Exposes an object with methods:
     - runAxe(root)
     - runContrast(root)
     - runFullChecks(root)
     - revalidateElement(root)
     - exportReport(obj, filename)

   This file supports:
     - AMD (define)
     - CommonJS (module.exports)
     - Browser global (window.A11yTool)
*/
(function (root, factory) {
  ...
  function findTextElementsAndCheckContrast(root) {
    root = root || document;
    const nodes = Array.from(root.querySelectorAll('p,span,a,li,button,label,h1,h2,h3,h4,h5,h6,td,th'));
    const results = [];
    for (const el of nodes) {
      try {
        const style = window.getComputedStyle(el);
        if (!style || style.visibility === 'hidden' || style.display === 'none' || parseFloat(style.opacity) === 0) continue;
        const text = el.textContent && el.textContent.trim(); if (!text) continue;
        const fg = parseComputedRGB(style.color); if (!fg) continue;
        let bgColor = null; let node = el; let complexBackground = false;
        while (node && node !== document.documentElement) {
          ...
          if (bgImage && bgImage !== 'none') complexBackground = true;
  ...
  async function runAxe(root) {
    if (typeof axe === 'undefined') throw new Error('axe-core not available');
    const target = root || document;
    const res = await axe.run(target);
    return res;
  }

  async function runFullChecks(root) {
    const results = { axe: null, contrast: null };
    try { results.axe = await (async () => { if (typeof axe !== 'undefined') return runAxe(root); return null; })(); } catch(e) { results.axe = { error: String(e) }; }
    try { results.contrast = findTextElementsAndCheckContrast(root); } catch(e) { results.contrast = { error: String(e) }; }
    return results;
  }
  ...
  function exportReport(obj, filename) {
    try {
      ...
      const a = document.createElement('a'); a.href = url; a.download = filename || 'a11y-report.json'; a.click(); URL.revokeObjectURL(url);
    } catch (e) { throw e; }
  }

  async function revalidateElement(root) {
    // returns { axe, contrast }
    return runFullChecks(root);
  }

  var API = {
    runAxe: runAxe,
    runContrast: findTextElementsAndCheckContrast,
    runFullChecks: runFullChecks,
    exportReport: exportReport,
    revalidateElement: revalidateElement
  };
